...
$.extend({
    tpajax: function tpajax(options) {
...
function TmptJs(configoptions) {
    ...
    var _renderFun = {
        ...
        toFixed: function toFixed(data, options) {
            return data.toFixed(parseInt(options));
        },
        maxLength: function maxLength(v, len, type) {
            return v.substring(0, len) + (type || '');
        },
        count: function count(arr) {
            //tmpt-show="{{arr.length>1}}"
            return arr.length;
        },
        sum: function sum(arr, pro) {
            var sum = 0;
        ...
        countif: function countif(arr, strif) {
            //strif="s=>s.a==s.b"
            strif = strif.replace("&gt;", ">");
        ...
        replaceIf: function replaceIf(strIf, strR) {
            try {
                if (eval(str)) {
                    return strR;
                }
        ...
        buildHtml: function buildHtml(str) {
            return str || '<div><h1>HHHHHHHHHHHHHHHHHH</h1></div>';
        },
        buildText: function buildText(str) {
        ...
        notExit: function notExit(value) {
            !_renderFun.exit(value);
        },
        isNull: function isNull(value) {
            return !!value;
        },
        "default": function _default(value, adefault) {
    ...
    //内部基础方法
    var _baseFun = {
        doArrFun: function doArrFun(arr) {
        ...
        allready: function allready(hasReady, tmptId) {
            if (hasReady) {
                //完成当前tmpt渲染;++
                _renderedNum++;
                //执行进度变化事件;
        ...
        //渲染自定义样式
        styleRender: function styleRender(tmptid) {
            var tmpt = $m("#" + tmptid);
        ...
        //渲染特性；
        attrRender: function attrRender(tmptid) {
            var $tmpt = $m("#" + tmptid);
            $show = $tmpt.find("[tmpt-show]"); //将内部的tmpt-show做处理          
            $show.each(function (i, v) {
                var attrV = $(this).attr("tmpt-show");
                if (attrV == "true" || attrV == "!false") {
                    //
                } else if (attrV == "false" || attrV == "!true") {
                        $(this).remove();
                    } else {
                        if (!eval(attrV)) {
                            $(this).remove();
        ...
        theif: function theif(str) {
            //str:   "a==b&((c>=3|d<1)&e!=2)|(f>4&e<='a')"
            ...
            $.each(arr, function (i, val) {
                $.each(val.split('&'), function (j, v) {
                    if (v.indexOf("==") > 0) {
                        var s = v.split("==");
                        s[0] = s[1];
                    } else if (v.indexOf(">=") > 0) {} else if (v.indexOf("<=") > 0) {} else if (v.indexOf(">") > 0 & v.indexOf(">=") < 0) {} else if (v.indexOf("<") > 0 & v.indexOf("<=") < 0) {}
                });
            });
        },
        //html编码
        ...
        //html解码
        htmlDescode: function htmlDescode(str) {
            if (typeof str === "string") {
                return str.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&apos;/g, '\'');
            } else {
                return str;
        ...
        //渲染结束 样式改变
        endLoad: function endLoad(tmptid) {
            var $tmpt = $m("#" + tmptid);
        ...
        replaceFun: function replaceFun() {},
        ...
        //序列化转对象
        serializToObj: function serializToObj(routeValue) {
            var obj = {};
         ...
         */
        getRenderProcess: function getRenderProcess() {
            //默认执行引擎就已经渲染50%；
         ...
         * 渲染出错的模块 
         */
        renderError: function renderError(tmptid) {
         ...
         * @returns {Bool} 验证结果
         */
        apiDataStatus: function apiDataStatus(data) {
         ...
         */
        apiDataProcessing: function apiDataProcessing(data) {
            return data;
        }
    };
    //主题
     ...
     * 放置模块
     */
    var putTmpt = function putTmpt(tmptId, $tmpt) {
        if (!_$document.find("#" + tmptId).length) _$document.append($tmpt);
    };
     ...
     */
    var moduleRender = function moduleRender(tmptid, controller, action, routevalue, asyncflag) {
        ...
        try {
            ...
            var renderdata = function renderdata(data) {
                if (data) {
                    ...
                    if (data.Type === 0) {
                        ...
                        data.$tmptid = tmptid; //"'" + tmptid + "'";  //TODO "'" + tmptid + "'"; 加单引？？？ 用于两个filter   去掉单引，用[]
                        var obj;
            ...
            if (!!options.data && typeof options.data === "object") {
                ...
                if (typeof routevalue === "string") options.url = controller + "/" + action + "?" + routevalue;
     ...
     */
    var build = function build($module) {
        // routevalue ->'moduleid=干教资讯&page=1&rows=6&sort=Id&order=desc&ids=9&wordLimt=28,moduleid=最新资讯&page=1&rows=6&sort=Id&order=desc&ids=7&wordLimt=28'
        //如果该模版是容器模版
     ...
     * @returns {Bool}
     */
    var renderBlock = function renderBlock($value, Obj, id) {
        //检验块的合理
        var $block = $value.parents("[tmpt-show]");
        if ($block.length > 0) {
            var showIf = $block.attr("tmpt-show").replace(/{{/g, '').replace(/}}/g, '');
        ...
        if (thisShow) {
            try {
                thisShow = thisShow.replace(/{{/g, '').replace(/}}/g, '');
     ...
     * @returns {$} 
     */
    var each4 = function each4($s, Obj, id) {
        // if(id == "课程分类"){
        //     debugger;
        // }
     ...
     * @returns {Html} 
     */
    var eachone4 = function eachone4($value, ListData, id, Obj) {
        ...
        $.each(ListData, function (i, value) {
            ...
            if (typeof value === "string") {
                //支持字符串数组渲染
                html += render4($value, { $value: value }, id);
            } else {
                //支持对象数组渲染
                    ...
                    if (value.Nodes && value.Nodes.length) {
                        $.each(value.Nodes, function (i, val) {
                            if (val.Nodes && val.Nodes.length) {
                                //val.Nodes && val.Nodes.length        //判断数据是否多层(至少三层的数据结构,模板只需两层,同时数据为Nodes才会进行递归)
     ...
     * @returns {Html}
     */
    var render4 = function render4($code, data, tmptid) {
     ...
     * @returns {Html|Text}
     */
    var logic = function logic(value, data, tmptid) {
        //data是对象
            ...
            if (/[a-zA-Z]+\(/g.test(value) === true) {
                ...
                if (!!pa && pa.length > 1) {
                    ...
                    $.each(parms, function (index, value) {
                        var val = "";
                        if (value == "$value") {
                            //{{fun($value,$value.data1,data2,'str')}}
                            val = data;
                        } else if (/'[^']*'/.test(value)) {
                            //字符串
                            val = value.match(/'([^']*)'/)[1];
    ...
    var displayFilter = function displayFilter(html, data) {
        var isDisplay;
    ...
    var customFunction = function customFunction(html, data) {
        var $source = $($.parseHTML(html.replace(/\r\n/g, "")));
    ...
    //兼容函数
    var compatible = function compatible() {
    ...
    */
    ...
    this.active = function (tmptid, time) {
        //time 用来激活延迟时间
...
function alertMs(options, opt_warntype, opt_callback1, opt_callback2) {
    ...
    var generate = {
        HTML: function HTML(option) {
            var $html = "" + "<div id=\"msOut\">" + "    <div id=\"msBox\">" + "        <div class=\"msTitle\">" + option.Title + "</div>" + "        <div class=\"msMessage\">" + option.Message + "</div>" + "        <div class=\"msBtn\">" + "            <span class=\"msConfirm\">确定</span>" + (option.warnType == 2 ? "<span class=\"msReject\">取消</span>" : "") + "        </div>" + "    </div>" + "    <div class=\"msLayer\"></div>" + "</div>";
            $('body').append($html);
            this.CSS(option);
        },
        ...
        Event: function Event(opt_callback1, opt_callback2) {
...
$dom.renderFun("examIndex", function (data, type) {
    ...
    var i = 0;
    i = (data.Type0Questions || []).length > 0 ? type === "Type0Questions" ? i : (++i, (data.Type1Questions || []).length > 0 ? type === "Type1Questions" ? i : (++i, (data.Type2Questions || []).length > 0 ? type === "Type2Questions" ? i : (++i, (data.Type3Questions || []).length > 0 ? type === "Type3Questions" ? i : i : ++i)
    //Type2Questions 不存在
    : (data.Type3Questions || []).length > 0 ? type === "Type3Questions" ? i : i : ++i)
    //Type1Questions 不存在
    : (data.Type2Questions || []).length > 0 ? type === "Type2Questions" ? i : (++i, (data.Type3Questions || []).length > 0 ? type === "Type3Questions" ? i : i : ++i) : ++i)
    //Type0Questions 不存在
    : (data.Type1Questions || []).length > 0 ? type === "Type1Questions" ? i : (++i, (data.Type2Questions || []).length > 0 ? type === "Type2Questions" ? i : (++i, (data.Type3Questions || []).length > 0 ? type === "Type3Questions" ? i : i : ++i) : ++i) : ++i;
    return index[i];
});
