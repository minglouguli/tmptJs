 ...
 * 20170105 xia 0.42 修改页面引擎对象名称由$R改为$dom,修改渲染方法名称为$R,Tmpt为兼容方式，可以兼容前两个作用；
 * 20170106 xia 0.43 添加getData(tmptId)接口，建议少在html上存储临时变量，可以利用该接口。（因为只读，不会被污染，可以考虑扩展双向绑定,但可能导致安全问题）
 * 20170111 xia 0.44 添加渲染进度改变事件
 * 20170113 lang 0.45 添加图片渲染失败替换
 * 20170124 lang 0.46 添加数据递归呈现不同html
 * 20170125 xia 0.47 添加ajax失败显示图片（可以局部404）
 * 20170209 xia 0.48 添加tmpt-html
 * 20170214 xia 0.49 修改api数据验证与处理(200 404 500 401等)
 * 20170214 xia 0.50 添加渲染内存模块功能 访问方法 $dom.virtual({}).complete(function(){}); 获取内存模块 $dom.find(selector);
 * 20170226 xia 0.51 添加tmpt-value, 用于修饰input,从$dom内部保存input提交的值,区别于parameter(这个是提交的过滤条件）而这个是需要添加的数据
 *
 * 20170502 xia 0.6 添加tmpt-bind，用于寄存数据.
 */
;
...
function TmptJs(configoptions) {
    ...
    //渲染数据处理方法对象
    var _renderFun = {
        key: function (value, index, page, rows) {

            var key = 0;
            if (page) {
                key = index + (page - 1) * rows;
            }
            else {
                key = Object.keys(_bindKey).length + 1;
            }
            if (_bindKey[key]) {
                value = _bindKey[key];
            }
    ...
    //#region 该注释不能删除
    //渲染模版
    //var moduleRender3 = function (tmptid, controller, action, routevalue, asyncflag) {
    //    var hasReady = true;
    //    var options = {};
    //    try {
    ...
    //            jtmpt = document.getElementById(tmptid);
    //        }
    //        else {
    //            jtmpt = $("#" + tmptid + " #" + options.partialId)[0]; // document.getElementById(tmptid);
    //        }
    ...
    //        }
    //        var modulearr = [];
    //        if (_cache[tmptid]) {                       //先从缓存中读取模版的html;
    //            if (!options.partialId) {
    //                source = _cache[tmptid];
    //            }
    //            else {
    //                source = $(_cache[tmptid]).find("#" + options.partialId)[0].outerHTML.replace(/\r\n/g, "");
    //            }
    //        }
    //        else {
    //            source = jtmpt.outerHTML.replace(/\r\n/g, "");                 //解决ie8 的换行
    //            modulearr = build($m("#" + tmptid));
    //            _cache[tmptid] = source;                       //缓存中没有该模版则从文档中获得
    //            _$cache[tmptid] = $m("#" + tmptid);
    //        }
    ...
    //                        $tmpt.find(" .content, .tmpt-load").css({
    //                            "background": "url(../images/1392662591495_1140x0.gif)"
    //                        });
    ...
    //                    //}
    //                    // each块渲染
    //                    var html = each3(source, obj, tmptid);

    //                    //each块外部渲染
    //                    jtmpt.outerHTML = render(html, data, tmptid);
    ...
    //                        jtmpt.outerHTML = render(source, "", tmptid);
    //                    },
    //                    complete: function () {
    //                        $m("#" + tmptid).addClass("rendered");
    //                        _baseFun.allready(hasReady, tmptid);
    ...
    //------------------------------------------------------
    /**
     * 循环处理  对source检查是否有each 如果有则对所有each块渲染（不渲染each外部分） 返回渲染结果；
     * @param {type} source outerHTML
     * @param {type} Obj
     * @param {type} id
     * @returns {type} 
     */
    //var each3 = function (source, Obj, id) {
    //    // var $s = $($.parseHTML(source));
    //    var $s = $($.parseHTML(source.replace(/\r\n/g, "")));               //for ie8
    ...
    //    $each.each(function () {
    //        var $the = $(this);
    //        // if (renderBlock($the, Obj, id)) {

    //        //model嵌套子model处理
    //        var modeldif = $the.attr("each-sonModel") || $the.attr("tmpt-each") || "ListData";
    //        // modeldif = Tmpt.render(modeldif, Obj, id);暂不实现tmpt-each可配置功能 ////TODO 20161112 tmpt-each配置功能；
    //        //if ($(this).attr("each-sonModel")) {
    //        var str = "Obj." + modeldif;
    //        var sonModel = eval(str) || "";
    //        // var sonModel = Obj[modeldif]||"";
    //        if (sonModel !== "" && sonModel.length > 0) {
    //            $the.replaceWith(eachone3($the.prop("outerHTML").replace(/\r\n/g, ""), sonModel, id, Obj));
    //        }
    //            //else if (IsArray(Obj)) {
    //            //    $(this).replaceWith(Tmpt.eachone3(this.outerHTML, Obj, id, null));
    //            //}
    //        else {
    //            //$the.html("");
    //            $the.remove();            //TODO 20161205 修改为直接移除
    //        }

    //        //var $model = $(this).parent().find("[each-sonModel]");
    //        //if ($model.length > 0) {
    //        //    if ($(this).attr("each-sonModel")) {
    //        //        var sonmodel = $(this).attr("each-sonModel") || "Nodes";
    //        //        var str = "ListData." +sonmodel;
    //        //        var sonModel = eval(str);
    //        //        if (sonModel != null) {
    //        //            $(this).replaceWith(Tmpt.eachone3(this.outerHTML, sonModel));
    //        //        }
    //        //        else {
    //        //            取到的数据为null
    //        //        }
    ...
    //        //}
    //        //else {
    //        //    $(this).replaceWith(Tmpt.eachone3(this.outerHTML, ListData));    //用渲染完成的each块替换原来未渲染的。
    //        //}
    //        //  }
    //    });
    //    return $s[0].outerHTML;                                          //返回each被渲染的source；
    //};

    //------------------------------------------------------
    //source:each outerHTML ;ListData:仅为被渲染的数据/数组;      单个each块渲染  返回渲染结果；
    //var eachone3 = function (source, ListData, id, Obj) {
    //    //var $value = $(source);
    //    var $value = $($.parseHTML(source.replace(/\r\n/, "")));   //for ie8
    ...
    //            html += render($value[0].outerHTML, { $value: value }, id);
    //        }
    //        else {                                             //支持对象数组渲染
    //            value.$index = i + 1;                         //“$index” 用来循环时，提供序号,在each内使用：{{$index}}
    //            value.$last = Obj;
    //            if ($$each.length > 0) {                      //可以补充个数据验证
    //                var code = each3($value[0].outerHTML, value, id);
    //                html += render(code, value, id);
    //            }
    //            else {
    //                html += render($value[0].outerHTML, value, id);
    //            }
    //            //////
    //        }
    //    });
    ...
    //---------------------------------------------
    //渲染处理；code:html字符串；data:用来渲染的数据；
    //var render = function (code, data, tmptid) {
    //    if (!code) {
    //        return "";
    //    }
    //    var mainCode = "";
    ...
    this.getBind = function (key) {
        return _bindKey[key];
    }

    this.setBind = function (key, value) {
        if (key) {
            _bindKey[key] = value;
        }
        else {
